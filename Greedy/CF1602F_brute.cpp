...
struct segmemt
{
    ...
    void build(){for(int i=0;i<maxn<<3;++i)t[i].mx=t[i].tag=0;}
...
int main()
{
    ...
    while(t--){
        ...
        for(int i=n;i>=1;i--)
        {
            ...
            printf("(%d %d)\n",p[i].b,p[i].a);
            if(p[i].a<=p[i].b){
                tr.change(1,1,cnt,p[i].a,p[i].b,p[i].w);
                printf("[%d %d] += %d\n",p[i].a,p[i].b,p[i].w);
            }
            ll mx=tr.query(1,1,cnt,max(p[i].a,p[i].b+1),cnt);
            printf("mx : max[%d %d] = %lld\n",max(p[i].a,p[i].b+1),cnt,mx);
            ll bv=tr.query(1,1,cnt,p[i].b,p[i].b);
            printf("bv : max[%d %d] = %lld\n",p[i].b,p[i].b,bv);
            if(mx+p[i].w>bv){
                tr.change(1,1,cnt,p[i].b,p[i].b,mx+p[i].w-bv);
                printf("[%d %d] += %lld\n" , p[i].b,p[i].b,mx+p[i].w-bv);
            }
        }
        printf("%lld\n",tr.t[1].mx);
    }
